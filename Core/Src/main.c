...
void taskOLED(void *pvParm){
  ...
  DHT11Sensor dht11;
	while(1){
		...
		BaseType_t dht11Receive = xQueueReceive(queueDHT11, &dht11, 0);
		if(dht11Receive == pdPASS){
			snprintf(displayTemp, sizeof(displayTemp), "Temp: %d.%d C", dht11.intTemp, dht11.floatTemp);
			snprintf(displayHumi, sizeof(displayHumi), "Humi: %d.%d %%", dht11.intHumi, dht11.floatHumi);
			u8g2_ClearBuffer(&u8g2);
...
void taskDHT11(void *pvParm){
  DHT11Sensor dht11;
	while(1){
		...
		if(DHT11GetData(&dht11) == 0){
			xQueueSend(queueDHT11, &dht11, 0);
			/*printf("Temp:%.1f C\r\n", dht11DATA.Temp);
			printf("Humi:%.1f %%\r\n", dht11DATA.Humi);*/
		}
...
/*
 * Problem description: The float formatting support is not enabled, check
your MCU Settings from "Project Properties > C/C++ Build > Settings >
Tool Settings", or add manually "-u _printf_float" in linker flags.
 * */
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
	queueDHT11 = xQueueCreate(3, sizeof(DHT11Sensor));
	/***********************
	memory allocate
